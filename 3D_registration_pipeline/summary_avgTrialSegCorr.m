...
hitCorrAndDist_all = hitCorrAndDist_all(hitCorrAndDist_all(:,1) < 1,:); %filter the pairs by correlation coefficient
...
plot(distanceBin(1:end-1)+2.5,corrBin,'Color',plotColor,'LineWidth',5);  %plot the average correlation coefficient of the distance bin
...
for i = 1:surrogateTimes
    ...
    tempSurroMat =tempSurroMat(tempSurroMat(:,1) < 1,:);  %exclude the cell pair with more than 0.5 CC
...
%remove the 0 of distanceBin
distanceBin1 = distanceBin(2:end);
realCorrBin10 = corrBin(find(distanceBin1 <= 10)); %get the real data <10 um bins
realCorrBin20 = corrBin(find(distanceBin1 > 10 & distanceBin1 <= 20 ));
realCorrBin30 = corrBin(find(distanceBin1 > 20 & distanceBin1 <= 30 ));
realCorrBin40 = corrBin(find(distanceBin1 > 30 & distanceBin1 <= 40 ));
...
figure;
histogram(diffSurro30_40,20,'FaceColor',[0.8 0.8 0.8],'LineWidth',2);
...
histogram(diffSurro10_40,20,'FaceColor',[0.8 0.8 0.8],'LineWidth',1.5);
xlabel('CC_<_1_0 _µ_m - CC_3_0_-_4_0 _µ_m');
ylabel('Number of Surrogates');
xlim([-0.05 0.18])
ylim([0 200])
hold on
twoTailP = mean(diffSurro10_40 >= diffRealCorrBin10_40);
xline(diffRealCorrBin10_40,'-',{num2str(twoTailP)},'Color',plotColor,'LineWidth',2);
hold off
set(gca,'FontSize',16)
set(gca,'LineWidth',1.5)
titleStr = append(behStr,'_',num2str(contrastLevel*100),'_pairsIn10');
print(titleStr, '-depsc', '-painters');
varStr = append('corrAndDist_',titleStr);
tempVar = hitCorrAndDist_all((hitCorrAndDist_all(:,2) <=10),:); %get the cell pairs within 20 um tangential distance
save(titleStr,'tempVar')
